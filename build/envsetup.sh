...
- pixelgerrit:     A Git wrapper that fetches/pushes patch from/to ShapeShiftOS Gerrit Review.
...
- repopick:        Utility to fetch changes from ShapeShiftOS Gerrit.
...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        echo "Found device"
        if (adb shell getprop org.ssos.device | grep -q "$CUSTOM_BUILD"); then
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop org.ssos.device | grep -q "$CUSTOM_BUILD");
    then
...
function pixelrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "ShapeShiftOS Gerrit Rebase Usage: "
...
function dopush()
{
    ...
    if (adb shell getprop org.ssos.device | grep -q "$CUSTOM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
